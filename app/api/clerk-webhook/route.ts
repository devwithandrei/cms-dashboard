...
export async function POST(req: Request) {
  ...
  // If webhook secret is provided, verify the payload
  if (WEBHOOK_SECRET) {
    // Get the headers
    ...
    // If there are no headers, skip verification
    if (svix_id && svix_timestamp && svix_signature) {
      ...
      // No headers, parse the body directly
      evt = JSON.parse(body) as WebhookEvent;
    }
  } else {
    // No webhook secret, parse the body directly
    evt = JSON.parse(body) as WebhookEvent;
  }
